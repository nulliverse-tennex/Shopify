class ProductForm extends HTMLElement {
  ...
  _updateVariant(variant) {
    ...
    this._updateStockCounter(variant);
    this._hideError();
  }
  ...
  _updateStockCounter(variant) {
    var counterEl = this.querySelector('[data-stock-counter]');
    if (!counterEl) return;

    var threshold = parseInt(counterEl.getAttribute('data-stock-threshold'), 10) || 10;
    var inventory = variant.inventory_quantity || 0;

    if (variant.available === false || inventory <= 0) {
      // Out of stock
      counterEl.className = 'stock-counter stock-counter--out';
      counterEl.textContent = '> ERROR: OUT OF STOCK_';
      counterEl.setAttribute('data-inventory-quantity', '0');
      counterEl.style.display = 'block';
    } else if (inventory <= threshold) {
      // Low stock warning
      counterEl.className = 'stock-counter stock-counter--low';
      counterEl.textContent = '> WARNING: Only ' + inventory + ' left in stock_';
      counterEl.setAttribute('data-inventory-quantity', inventory);
      counterEl.style.display = 'block';
    } else {
